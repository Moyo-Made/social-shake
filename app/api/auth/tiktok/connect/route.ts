...
export async function GET(req: NextRequest) {
	try {
		...
		await setStateSession(csrfState, userId);

		console.log(
			`Setting CSRF state in session: ${csrfState} for user: ${userId}`
		);

		const tiktokAuthUrl = new URL("https://www.tiktok.com/v2/auth/authorize/");
		tiktokAuthUrl.searchParams.append("client_key", CLIENT_KEY);
		tiktokAuthUrl.searchParams.append("response_type", "code");
		// Request only the scopes that are configured in your TikTok developer app
		tiktokAuthUrl.searchParams.append(
			"scope",
		...
		const errorMessage =
			error instanceof Error
				? error.message
				: "Failed to initialize TikTok connection";
