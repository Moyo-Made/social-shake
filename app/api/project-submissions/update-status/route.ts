...
export async function POST(request: NextRequest) {
  try {
    ...
    const { submissionId, status, sparkCode, tiktokLink, feedback } = requestData;
    ...
    const updatedSubmissionData = {
      ...
      ...(updatedSubmissionDoc.data() ? convertFirestoreDataToClientFormat(updatedSubmissionDoc.data()!) : {})
    };
    ...
    if (status === "spark_submitted" || status === "spark_verified") {
      ...
      await adminDb.collection("notifications").add({
        type: status === "spark_submitted" ? "spark_code_submitted" : "spark_code_verified",
        submissionId,
...
function convertFirestoreDataToClientFormat(data: FirebaseFirestore.DocumentData) {
  if (!data) return {};
