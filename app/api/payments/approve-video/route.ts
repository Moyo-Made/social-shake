			...
			try {
				...
				if (deliverableDoc.exists) {
					await deliverableRef.update({
						...
						approved_by: paymentData.brandId || paymentData.user_id,
			...
			try {
				...
				updatedPaymentData = {
					...
					approvalDetails: {
						...
						approvedBy: paymentData.brandId || paymentData.user_id,
						approvedAt: new Date().toISOString()
					}
				};
					...
					try {
						...
						if (deliverableDoc.exists) {
							await deliverableRef.update({
								...
								rejected_by: paymentData.brandId || paymentData.user_id,
