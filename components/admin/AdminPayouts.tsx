...
export default function AdminPaymentsDashboard() {
	...
	const calculateTotals = (paymentData: Payment[]) => {
		let total = 0;
		let pending = 0;
		let processed = 0;
		...
		paymentData.forEach((payment) => {
			...
			if (payment.status === "pending_capture") {
		...
		setTotalPayments({
			...
			processedAmount: processed.toFixed(2),
		});
	};
			...
			if (response.data.success) {
				...
				const updatedPayments = payments.map((payment) =>
					payment.id === paymentId
						? ({
								...payment,
								status: action === "capture" ? "completed" : "canceled",
								stripePaymentStatus:
									action === "capture" ? "succeeded" : "canceled",
							} as Payment)
						: payment
				);
	...
	const filteredPayments = useMemo(() => {
		...
		return payments.filter((payment) => {
			...
			const matchesStatus =
				...
				(statusFilter === "Processed" && payment.status === "completed") ||
				(statusFilter === "Pending" && payment.status === "pending_capture") ||
				(statusFilter === "Refunded" && payment.status === "canceled");

			return matchesSearch && matchesType && matchesStatus;
		});
	}, [searchTerm, typeFilter, statusFilter, hasPayments, payments]);
