...
export default function ContestGridCard({ contest }: ContestGridCardProps) {
	...
	// Make sure we use the contest.id property
	...
	return (
		<div className="border border-[#6670854D] rounded-lg bg-white p-6 ">
			<div className="flex justify-between items-start mb-4">
				...
				<span
					className={`rounded-full px-2 py-1 text-sm flex items-center ${className}`}
				>
			...
			<div className="flex items-center mb-6">
				...
				<span className="ml-auto text-sm text-gray-500">
					{getContestTypeText(contest.contestType || "")}
				</span>
			</div>

			<div className="grid grid-cols-3 gap-4">{renderMetrics()}</div>
			...
			<Button className="w-full bg-orange-500 hover:bg-orange-600 text-white font-medium py-3 px-4 rounded-lg mt-6 transition-colors">
				<Link
					href={contestId ? `/creator/dashboard/contest/${contestId}/leaderboard` : "#"}
					className="w-full h-full flex items-center justify-center"
				>
					{contestId ? "View Leaderboard" : "Contest ID Missing"}
				</Link>
			</Button>
		</div>
	);
